...
describe EmployerFreelancersHelper, type: :helper do
  before(:each) do
  ...
  it 'instantiates' do
    expect(EmployerFreelancersHelper::Filter.new).to be_truthy
  end

  describe '#valid?' do
    context 'returns true if' do
      it 'a valid sector_id is passed' do
        expect(EmployerFreelancersHelper::Filter.new(sector_ids:[obscure_sector.id]).valid?).to be_truthy
      end
      it 'a valid skill_id is passed' do
        expect(EmployerFreelancersHelper::Filter.new(real_estate_skill_ids:[obscure_real_state_skill.id]).valid?).to be_truthy
      end
      it 'a valid software_id is passed' do
        expect(EmployerFreelancersHelper::Filter.new(software_ids:[obscure_software.id]).valid?).to be_truthy
      end
    end

    context 'returns false if' do
      it 'nothing is passed' do
        expect(EmployerFreelancersHelper::Filter.new().valid?).to be_falsey
      end

      it 'empty arrays are passed' do
        expect(EmployerFreelancersHelper::Filter.new(sector_ids: [], real_estate_skill_ids: [], software_ids: []).valid?).to be_falsey
      end

      it 'no valid ids are passed' do
        expect(EmployerFreelancersHelper::Filter.new(sector_ids: [999], real_estate_skill_ids: [888], software_ids: [777]).valid?).to be_falsey
      end
    end
  end
  ...
  describe '#freelancer_profile_ids' do
    ...
    context 'only one object of one type is passed' do
      let!(:filter) { EmployerFreelancersHelper::Filter.new(sector_ids: [obscure_sector.id]) }
    ...
    context 'one tag each for all the filters' do
      ...
      let(:filter) { EmployerFreelancersHelper::Filter.new(sector_ids: [obscure_sector.id], real_estate_skill_ids: [obscure_real_state_skill.id], software_ids: [obscure_software.id]) }
    ...
    context 'two tags of one type of filter' do
      ...
      let(:filter) { EmployerFreelancersHelper::Filter.new(sector_ids: [obscure_sector.id, obscure_sector2.id]) }
    ...
    context 'two tags of two types of filters' do
      ...
      let(:filter) { EmployerFreelancersHelper::Filter.new(sector_ids: [obscure_sector.id, obscure_sector2.id], software_ids: [obscure_software.id, obscure_software2.id]) }
