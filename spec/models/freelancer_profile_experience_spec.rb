...
RSpec.describe FreelancerProfileExperience, type: :model do
  ...
  let(:freelancer_profile_experience_current) { create(:freelancer_profile_experience, :current, freelancer_profile: freelancer_profile) }

  it 'factory works' do
    expect(freelancer_profile_experience).to be_valid
    expect(freelancer_profile_experience_current).to be_valid
  end
  ...
  context 'Validations' do
    ...
    describe 'custom validation in #end_date_is_after_start_date' do
      let(:invalid_experience) { FactoryBot.create(:freelancer_profile_experience, freelancer_profile: freelancer_profile, start_year: 2015, end_year: 2014) }
      let(:blank_experience) { FactoryBot.create(:freelancer_profile_experience, freelancer_profile: freelancer_profile, start_month: nil, start_year: nil, end_month: nil, end_year: nil) }

      it 'prevents object create if ending date occur before the starting date' do
        expect { invalid_experience }
      ...
      it 'allows the update if dates are the same' do
        expect { freelancer_profile_experience.update!(start_month: 2, start_year: 2015, end_month: 2, end_year: 2015) }.not_to raise_error
      end

      it 'allows the update if end_month and end_year absent' do
        expect { freelancer_profile_experience.update!(start_month: 2, start_year: 2015, end_month: nil, end_year: nil) }.not_to raise_error
      end

      it 'allows the update if all dates absent' do
        expect { freelancer_profile_experience.update!(start_month: nil, start_year: nil, end_month: nil, end_year: nil) }.not_to raise_error
      end

      it 'allows the object create if all dates absent' do
        expect { blank_experience }.not_to raise_error
      end
    end
  end
