  ...
  ActiveAdmin.register FreelancerProfile do
    ...
    show title: 'Freelancer Profile' do |freelancer_profile|
      attributes_table do
        ...
        row 'Contracts' do
          freelancer_profile.user.received_contracts.map { |contract| link_to("Hired by #{contract.from_user.email} for $#{contract.pay_rate} #{contract.contract_type}", admin_contract_path(contract.id)) }.join('<br>').html_safe
        end
      end
