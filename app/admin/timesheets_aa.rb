    ...
    show title: 'Timesheet' do |timesheet|
      attributes_table do
        ...
        row 'Billing Entries' do
          (timesheet.billings.order(work_done: :desc).map{|billing| link_to(billing.description, admin_billing_path(billing.id))}.join('<br>') + link_to('Add New', new_admin_billing_path(:billing=> { :contract_id => timesheet.contract_id, :timesheet_id => timesheet.id }), target: '_new')).html_safe
        end
